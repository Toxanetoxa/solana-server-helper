...
- Перед каждым коммитом запускается `pnpm exec lint-staged` (форматирует Prettier и прогоняет ESLint по staged-файлам) и `pnpm test`; при любой ошибке commit отменяется.
